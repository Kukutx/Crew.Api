...
internal sealed class GetFeedQueryHandler : IGetFeedQueryHandler
{
    ...
    public GetFeedQueryHandler(AppDbContext dbContext)
    {
    ...
    public async Task<GetFeedResult> HandleAsync(GetFeedQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var baseQuery =
            ...
            where EF.Functions.STDWithin(e.Location, origin, radiusMeters)
            ...
            let distanceKm = EF.Functions.Distance(e.Location, origin) / 1000d
        ...
        {
            baseQuery = baseQuery.Where(x => x.Event.Tags.Any(t => t.Tag != null && normalizedTags.Contains(t.Tag!.Name.ToUpperInvariant())));
        }
